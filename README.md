...
- 管理页面：`http://服务器地址:端口/ui`
- Swagger UI：`http://服务器地址:端口/docs`
- OpenAPI JSON：`http://服务器地址:端口/openapi.json`

管理页面由服务端直接提供静态资源，无需单独构建或部署前端。它支持创建任务、选择多张图片或递归选择图片目录、查看模型状态与任务进度、筛选任务，以及下载或删除结果。页面会根据浏览器窗口在双栏、单栏和移动端任务卡片布局之间自动切换。

任务列表每 60 秒自动刷新一次，并按创建时间从新到旧显示。页面只为 `uploading`、`queued` 和 `running` 任务调用进度接口；`succeeded` 任务直接显示 `100%`，不会继续请求其进度。浏览器页面不可见时暂停自动请求，重新显示时立即刷新一次。页面中的 API Key 只保存在当前页面内存中，刷新页面后会清除。
